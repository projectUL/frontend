...
function RegisterForm() {
  ...
  function validation() {
    ...
    if (isEmployer && formData.companyName.value.length < 2) errorObj.companyName = true;
    if (!validateEmail(formData.email.value)) errorObj.email = true;
    if (!checkPassword(formData.password.value)) errorObj.password = true;
    if (formData.password.value !== formData.repeatPassword.value) errorObj.repeatPassword = true;
          ...
          <div className={classes.inputs}>
            {isEmployer && (
              <div>
                ...
                {showError.companyName && <ErrorMessageForm message="Company name should have at least 2 characters." />}
              </div>
            )}
            <div>
              <label htmlFor="email">Email</label>
              <input name="email" type="text" id="email" value={formData.email.value} onChange={inputChangeHandler} />
              {showError.email && <ErrorMessageForm message="Email adress is not valid." />}
            </div>
            <div>
            ...
            <div>
              ...
              {showError.repeatPassword && <ErrorMessageForm message="Passwords doesn't match." />}
            </div>
          </div>
        </div>
